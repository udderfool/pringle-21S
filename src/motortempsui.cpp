...
static void gettemp(lv_event_t * e) {
    ...
    lv_obj_set_style_bg_opa(target, (temp-30)*10, LV_PART_MAIN);
...
void tempcheck() {
    ...
    int motorrow = 0;
    for(int b = 0; b < motorbar.size() && b < 12; b++){
        ...
        lv_obj_add_event_cb(motorview, getTemp, LV_EVENT_ALL, &motorbar[b].port);
    }
    lv_obj_add_style(mainlabel, &stylemotor, LV_PART_MAIN);
    //lv_obj_set_style_border_color(mainlabel, lv_color_hex(0x071808), LV_PART_MAIN);
...
void tempcheckctrl() {
  driveTemp =  
  (motorbar[1].motor.get_temperature() 
  + motorbar[2].motor.get_temperature() 
  + motorbar[3].motor.get_temperature() 
  + motorbar[5].motor.get_temperature() 
  + motorbar[6].motor.get_temperature() 
  + motorbar[7].motor.get_temperature())/6;
  ...
  wallmechTemp = motorbar[8].motor.get_temperature();
    if(master.get_digital(pros::E_CONTROLLER_DIGITAL_X)) {
        if(intakeTemp <= 30) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "intake: cool, %.0f°C", intakeTemp);
        else if(intakeTemp > 30 && intakeTemp <= 45) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "intake: warm, %.0f°C", intakeTemp);
        else if(intakeTemp > 45) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "intake: hot, %.0f°C", intakeTemp);
        pros::delay(2000);
    } else if(master.get_digital(pros::E_CONTROLLER_DIGITAL_UP)) {
        if(wallmechTemp <= 30) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "wallmech: cool, %.0f°C", wallmechTemp);
        else if(wallmechTemp > 30 && wallmechTemp <= 45) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "wallmech: warm, %.0f°C", wallmechTemp);
        else if(wallmechTemp > 45) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "wallmech: hot, %.0f°C", wallmechTemp);
        pros::delay(2000);
    } else {
        if(driveTemp <= 30) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "drive: cool, %.0f°C", driveTemp);
        else if(driveTemp > 30 && driveTemp <= 45) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "drive: warm, %.0f°C", driveTemp);
        else if(driveTemp > 45) pros::c::controller_print(pros::E_CONTROLLER_MASTER, 0, 0, "drive: hot, %.0f°C", driveTemp);
        pros::delay(2000);
    }
}