...
void jautonrun() {
    jautoncurated[selected].AutonCall();
}
...
static void jautoncurate(lv_event_t * e) {
    ...
    if((j_auton_selector.jasautontable[i].RedBluFilt == redblustore || j_auton_selector.jasautontable[i].RedBluFilt == 2) && (j_auton_selector.jasautontable[i].PosNegFilt == posnegstore || j_auton_selector.jasautontable[i].PosNegFilt == 2)) {
...
static void selectauton(lv_event_t * e) {
    ...
    if(strcmp(lv_table_get_cell_value(jauton, row, 0), "") != 0) {
        noselection = false;
    lv_obj_has_state(redblu, LV_STATE_CHECKED) == true ? lv_obj_set_style_bg_color(redbluind, lv_color_hex(0x0066cc), LV_PART_MAIN) : lv_obj_set_style_bg_color(redbluind, lv_color_hex(0xff2a00), LV_PART_MAIN);
    lv_obj_has_state(posneg, LV_STATE_CHECKED) == true ? lv_obj_set_style_bg_color(posind, lv_color_hex(0xdb8826), LV_PART_MAIN) : lv_obj_set_style_bg_color(negind, lv_color_hex(0x00b5bc), LV_PART_MAIN);
...
static void updownbtn(lv_event_t * e) {
    if(lv_obj_has_state(autonselectup, LV_EVENT_CLICKED)) {
        if(j<4) j = (jautoncurated.size()-(jautoncurated.size()%4));
        else j -= 4;
